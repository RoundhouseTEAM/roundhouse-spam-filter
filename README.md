...
- a `BLOCKED_KEYWORDS` term in the message or a typed extra field (2.13.0): `seo`, `digital marketing`, `vas 4 hire`, `virtual assistant(s)`, `wikipedia`, `free b2b data` (2.15.0). Multi-word terms also match run together ("VAs4Hire") and are checked in the name; single words like `seo` never are (Seo is a surname)
...
**Exception (Philip, 2026-09-22, extended 2026-09-23 and 2026-09-25):** `BLOCKED_KEYWORDS` — `seo`,
`digital marketing`, `vas 4 hire`, `virtual assistant`, `virtual assistants`, `wikipedia`,
`free b2b data` (also "FreeB2BData") — are
**withheld**, not flagged. Whole words in the message and typed extra fields; multi-word
terms also match run together and are checked in the name, single words never are
(Seo is a surname). Philip's call: these pitches are never a client's lead.
